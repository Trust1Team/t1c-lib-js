...
class EidBe extends GenericCertCard implements AbstractEidBE {
    ...
    public rnData(callback?: (error: RestException, data: RnDataResponse) => void): Bluebird<RnDataResponse> {
        return this.connection.get(this.baseUrl, this.containerSuffix(EidBe.RN_DATA), undefined, callback);
    }

    public address(callback?: (error: RestException, data: AddressResponse) => void): Bluebird<AddressResponse> {
