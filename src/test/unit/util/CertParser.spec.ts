import { CertParser } from './../../../scripts/util/CertParser';
/**
 *
 * @author Gilles Platteeuw
 * @since  2018
 */


describe('CertParser Utility', () => {
    let cert: string;
    let certs: string[];
    let certObject: {
        data: {
            certificate: string,
            otherProperty: string
        }
    };
    let certsObject: {
        data: {
            certificate: string[],
            otherProperty: string
        }
    };

    beforeEach(() => {
        cert = '';
        certs = [
            '',
            ''
        ];
        certObject = { data:
                {
                    certificate: cert,
                    otherProperty: 'this should be untouched'
                }
        };
        certsObject = {
            data: {
                certificate: certs,
                otherProperty: 'this should be untouched'
            }
        };
    });

    describe('it can process a certificate', () => {
        test('correctly processes a certificate', () => {
            let processed = CertParser.processCert(cert);
        });
    });

    describe('it only processes certificates if required', () => {
        test('skips parsing when not required', () => {
            return CertParser.process({ data: cert }, false).then(res => {
                expect(res).toHaveProperty('data');
                expect(res.data).toHaveProperty('base64');
                expect(res.data.base64).toEqual(cert);
                expect(res.data).not.toHaveProperty('parsed');
            });
        });

        test('parses certificates when asked', () => {
            return CertParser.process({ data: cert }, true).then(res => {
                expect(res).toHaveProperty('data');
                expect(res.data).toHaveProperty('base64');
                expect(res.data.base64).toEqual(cert);
                expect(res.data).toHaveProperty('parsed');
            });
        });
    });

    describe('it can process an array of certificates', () => {
        test('skips parsing when not required', () => {
            return CertParser.process({ data: certs }, false).then(res => {
                expect(res).toHaveProperty('data');
                expect(res.data[0]).toHaveProperty('base64');
                expect(res.data[1]).toHaveProperty('base64');
                expect(res.data[0].base64).toEqual(certs[0]);
                expect(res.data[1].base64).toEqual(certs[1]);
                expect(res.data[0]).not.toHaveProperty('parsed');
                expect(res.data[1]).not.toHaveProperty('parsed');
            });
        });

        test('parses certificates when asked', () => {
            return CertParser.process({ data: certs }, true).then(res => {
                expect(res).toHaveProperty('data');
                expect(res.data[0]).toHaveProperty('base64');
                expect(res.data[1]).toHaveProperty('base64');
                expect(res.data[0].base64).toEqual(certs[0]);
                expect(res.data[1].base64).toEqual(certs[1]);
                expect(res.data[0]).toHaveProperty('parsed');
                expect(res.data[1]).toHaveProperty('parsed');
            });
        });
    });

    describe('it correctly parses the certificates in a larger object', () => {
        test('detects the certificates in an object', () => {
            let originalCert = certObject.data.certificate;
            return CertParser.process(certObject, false).then(res => {
                expect(res).toHaveProperty('data');
                expect(res.data).toHaveProperty('otherProperty', 'this should be untouched');
                expect(res.data).toHaveProperty('certificate');
                expect(res.data.certificate).toHaveProperty('base64');
                expect(res.data.certificate.base64).toEqual(originalCert);
                expect(res.data.certificate).not.toHaveProperty('parsed');
            });
        });

        test('processes certificates in an object if required', () => {
            let originalCert = certObject.data.certificate;
            return CertParser.process(certObject, true).then(res => {
                expect(res).toHaveProperty('data');
                expect(res.data).toHaveProperty('otherProperty', 'this should be untouched');
                expect(res.data).toHaveProperty('certificate');
                expect(res.data.certificate).toHaveProperty('base64');
                expect(res.data.certificate.base64).toEqual(originalCert);
                expect(res.data.certificate).toHaveProperty('parsed');
            });
        });
    });

    describe('it correctly parses a certificate array in a larger object', () => {
        test('detects the certificates in an object', () => {
            let originalCert = certsObject.data.certificate;
            return CertParser.process(certsObject, false).then(res => {
                expect(res).toHaveProperty('data');
                expect(res.data).toHaveProperty('otherProperty');
                expect(res.data).toHaveProperty('certificate');
                expect(res.data.certificate[0]).toHaveProperty('base64');
                expect(res.data.certificate[0].base64).toEqual(originalCert[0]);
                expect(res.data.certificate[0]).not.toHaveProperty('parsed');
                expect(res.data.certificate[1]).toHaveProperty('base64');
                expect(res.data.certificate[1].base64).toEqual(originalCert[1]);
                expect(res.data.certificate[1]).not.toHaveProperty('parsed');
            });
        });

        test('processes certificates in an object if required', () => {
            let originalCert = certsObject.data.certificate;
            return CertParser.process(certsObject, true).then(res => {
                expect(res).toHaveProperty('data');
                expect(res.data).toHaveProperty('otherProperty', 'this should be untouched');
                expect(res.data).toHaveProperty('certificate');
                expect(res.data.certificate[0]).toHaveProperty('base64');
                expect(res.data.certificate[0].base64).toEqual(originalCert[0]);
                expect(res.data.certificate[0]).toHaveProperty('parsed');
                expect(res.data.certificate[1]).toHaveProperty('base64');
                expect(res.data.certificate[1].base64).toEqual(originalCert[1]);
                expect(res.data.certificate[1]).toHaveProperty('parsed');
            });
        });
    });


});
