...
import { LocalConnection, RemoteJwtConnection, LocalAuthConnection, LocalTestConnection, RemoteApiKeyConnection, LocalAuthAdminConnection } from './client/Connection';
...
var GCLClient = (function () {
    function GCLClient(cfg, automatic) {
        ...
        this.luxeid = function (reader_id, pin) {
            return _this.pluginFactory.createEidLUX(reader_id, pin);
        };
    ...
    GCLClient.prototype.download = function (callback) {
        ...
        return this.core().infoBrowser().then(function (info) {
            var downloadData = new DSDownloadRequest(info.data.browser, info.data.manufacturer, info.data.os, info.data.ua, _this.config().gwUrl);
    ...
    GCLClient.prototype.implicitDownload = function () {
        ...
        this.core().info(function (error) {
            ...
            if (error) {
                ...
                var downloadData = new DSDownloadRequest(_info.data.browser, _info.data.manufacturer, _info.data.os, _info.data.ua, self.config().gwUrl);
